...
def launch_epoch(
        ...
        with node_executor.batch():
            for wfiles in work_files:
                job = node_executor.submit(
                    sa.launch_heating, wfiles[0], wfiles[1], wfiles[2]
                )
                jobs.append(job)
    ...
    if config.general.pre_epoch_equil:
        ...
        with node_executor.batch():
            for wfiles in work_files:
                job = node_executor.submit(
                    sa.launch_equil, wfiles[0], wfiles[1], wfiles[2]
                )
                jobs.append(job)
            ...
            for idx in pending_jobs.copy():
                if jobs[idx].done():
                    ...
                    if result[0] == -1 and not resubmitted[idx]:
                        # Resubmit failed job
                        logger.info(
                            f"Resubmitting failed equilibration job for seed {idx} in epoch {epoch_num}"
                        )
                        new_job = node_executor.submit(
                            sa.launch_equil,
                            work_files[idx][0],
        ...
        for idx in pending_jobs.copy():
            if jobs[idx].done():
                ...
                if result[0] == -1 and not resubmitted[idx]:
                    # Resubmit failed job
                    logger.info(
                        f"Resubmitting failed production job for seed {idx} in epoch {epoch_num}"
                    )
                    new_job = node_executor.submit(
                        sa.launch_prod,
                        work_files[idx][0],
