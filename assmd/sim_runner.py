...
def check_log_for_string(
    directory: str, log_filename: str, search_string: str, max_lines: int = 100
) -> bool:
    """
...
def launch_epoch(
    ...
    outputs = []
    if config.general.pre_epoch_heating and config.general.pre_epoch_equil:
        # everything run
        jobs = []
        with node_executor.batch():
            for wfiles in work_files:
                job = node_executor.submit(
                    sa.launch_everything, wfiles[0].abs_path, wfiles[1], wfiles[2]
                )
                jobs.append(job)
        ...
        while pending_jobs:
            ...
            for idx in pending_jobs.copy():
                if jobs[idx].done():
                    ...
                    status, desc = validate_simulation_outputs(
                        work_files[idx][0].abs_path, "prod"
                    )
                ...
                for idx in pending_jobs.copy():
                    if jobs[idx].done():
                        ...
                        try:
                            ...
                            result = (-1, f"Exception: {str(e)}")  # Treat as failed job
                        status, desc = validate_simulation_outputs(
                            work_files[idx][0].abs_path, "heating"
                        )
                        if not status or result[0] != 0:
                            if not resubmitted[idx]:
                                # Resubmit failed job
                                ...
                                logger.error(
                                    f"job for seed {idx} was already resubmitted"
                                )
                ...
                for idx in pending_jobs.copy():
                    if jobs[idx].done():
                        ...
                        try:
                            ...
                            result = (-1, f"Exception: {str(e)}")  # Treat as failed job
                        status, desc = validate_simulation_outputs(
                            work_files[idx][0].abs_path, "equil"
                        )
                        if not status or result[0] != 0:
                            if not resubmitted[idx]:
                                # Resubmit failed job
        ...
        with node_executor.batch():
            for wfiles in work_files:
                job = node_executor.submit(
                    sa.launch_prod, wfiles[0].abs_path, wfiles[1], wfiles[2]
                )
                jobs.append(job)
