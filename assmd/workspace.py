...
def prepare_initial_workspace(config:conf.JobConfig)->fs.AdaptiveWorkplace:
    ...
    if not os.path.exists(config.working_dir):
        try:
            ...
            logger.critical(f"Cant access or create working directory {config.working_dir}")
            sys.exit(1)
    else:
    ...
    for i, seed in enumerate(zip(config.init.seed_parm_paths if len(config.init.seed_parm_paths)>1 else config.init.seed_parm_paths*len(config.init.seed_paths), config.init.seed_paths)):
        ...
        try:
            ...
            sys.exit(1)
        workspace.add_file(parm_file, tags=["topology", f"seed_{i}", "epoch_0"], protected=True)
        ...
        try:
            ...
            sys.exit(1)
        workspace.add_file(coords_file, tags=[f"seed_{i}", "coords","epoch_0"], protected=True)
        seed_info.write(f"seed_{i}\n\t{seed[0]} -> {parm_file}\n\t{seed[1]} -> {coords_file}\n")
    seed_info.close()
    ...
    for i in range(config.adaptive.num_seeds):
        tops.append(workspace.files[workspace.get_files_by_tags([f"seed_{i}", "topology"])].abs_path)
        crds.append(workspace.files[workspace.get_files_by_tags([f"seed_{i}", "coords"])].abs_path)
    test_coords_and_project(tops, crds, workspace.files[workspace.get_files_by_tags("projection")].abs_path)
    
    if len(workspace.get_files(workspace.get_files_by_tags("epoch_0_dir")))==1:
        for i in range(1,config.adaptive.num_seeds):
            shutil.copytree(os.path.join("inputs", f"seed_0"), os.path.join("inputs", f"seed_{i}"))
            tmp = set()
            for fid in workspace.files[workspace.get_files_by_tags(f"seed_0")]:
                to_register = os.path.join("inputs", f"seed_{i}", workspace.files[fid].filename)
...
def prepare_epoch_run(config:conf.JobConfig, workspace:fs.AdaptiveWorkplace, epoch_num:int): #check protected
    os.chdir(config.working_dir)
